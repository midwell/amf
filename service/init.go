...
func (amf *AMF) Start() {
	...
	if li := factory.AmfConfig.Configuration.Li; li != nil {
		...
		// other subsystem too: the service-based interface, NGAP, registration with the
	...
	if self.EnableDbStore {
		...
		// already happens.
		//
		// Fatalf, and here it is the whole of the cleanup there is to do: this
		// runs before the NRF registration below, before the NGAP listener and
		// before the SBI server, so there is no association to drain and no
		// registration to withdraw. The graceful path exists for a failure
		// after the AMF has started serving; this one cannot be.
